...
func TestRepository_Create(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "should return error when userID does not exist (FK constraint violation)",
			args: args{
				bookmark: &model.Bookmark{
					BaseModel:   model.BaseModel{ID: "fk-violation-bookmark"},
					Description: "FK Violation Bookmark",
					URL:         "https://example.com/fk",
					Code:        "codefkviolation",
					UserID:      "nonexistent-user-id",
				},
			},
			verify: func(t *testing.T, db *gorm.DB, err error, a args) {
				require.Error(t, err)
				assert.ErrorIs(t, err, dbutils.ErrForeignKeyViolationType)
